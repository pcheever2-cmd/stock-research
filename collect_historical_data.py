...
Collects 5 years of price + quarterly financial data from FMP
Stores in separate backtest.db to keep daily pipeline fast

Usage:
    python collect_historical_data.py                     # Collect everything
...
async def collect_prices_for_symbol(fetcher: AsyncFMPFetcher, symbol: str) -> Optional[list]:
    """Fetch 5 years of daily OHLCV for one symbol"""
...
async def collect_prices(fetcher: AsyncFMPFetcher, tracker: ProgressTracker,
    ...
    await collect_data_type(
        ...
        lambda f, s: collect_prices_for_symbol(f, s),
        save_prices_batch,
        "Prices"
    )
...
async def collect_financials(fetcher: AsyncFMPFetcher, tracker: ProgressTracker,
    ...
    financial_types = [
        ('income', lambda f, s: collect_financial_for_symbol(f, s, 'income'),
         save_income_batch, "Income Statements"),
        ('key_metrics', lambda f, s: collect_financial_for_symbol(f, s, 'metrics'),
         save_metrics_batch, "Key Metrics"),
        ('balance_sheet', lambda f, s: collect_financial_for_symbol(f, s, 'balance'),
         save_balance_batch, "Balance Sheets"),
        ('cash_flow', lambda f, s: collect_financial_for_symbol(f, s, 'cashflow'),
         save_cashflow_batch, "Cash Flows"),
    ]

    for data_type, fetch_fn, save_fn, label in financial_types:
        log.info("\n" + "=" * 60)
        log.info(f"COLLECTING: {label} (quarterly, 5 years)")
        log.info("=" * 60)
...
async def main():
    ...
    log.info(f"Mode: {'prices only' if args.prices_only else 'financials only' if args.financials_only else 'full collection'}")

    async with AsyncFMPFetcher(FMP_API_KEY) as fetcher:
        if not args.financials_only:
            await collect_prices(fetcher, tracker, symbols)

        if not args.prices_only:
            await collect_financials(fetcher, tracker, symbols)
