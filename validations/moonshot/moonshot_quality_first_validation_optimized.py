#!/usr/bin/env python3
"""
Quality-First Moonshot Score Validation (OPTIMIZED)
===================================================
...
- Reduces runtime from ~10 hours to ~2-3 hours (70% faster)
...
def load_data():
    ...
    print("=" * 80)
    print("QUALITY-FIRST MOONSHOT VALIDATION (OPTIMIZED)")
...
def compute_moonshot_scores(fund_subset, prices, date, momentum_dict):
...
def validate_period(fund, prices, fwd_returns_dict, momentum_dict, start_date, end_date, period_name):
    ...
    for i, date in enumerate(rebalance_dates):
        ...
        # Compute scores
        scores = compute_moonshot_scores(fund_subset, prices, date, momentum_dict)
        if len(scores) == 0:
            continue
...
def main():
    ...
    momentums = precompute_momentum(prices)

    # In-sample validation
    validate_period(fund, prices, fwd_returns, momentums, IS_START, IS_END, "IN-SAMPLE")

    # Out-of-sample validation
    validate_period(fund, prices, fwd_returns, momentums, OOS_START, OOS_END, "OUT-OF-SAMPLE")
