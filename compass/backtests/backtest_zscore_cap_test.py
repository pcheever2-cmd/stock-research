...
def compute_compass_scores(fund_subset, prices, date, volatility_dict, apply_filters=False, zscore_cap=3.0):
    """
    ...
    Args:
        ...
        zscore_cap: Z-score cap level (default 3.0)
    """
    results = []
...
def backtest_period(fund, prices, fwd_returns_dict, volatility_dict, start_date, end_date, period_name, apply_filters=False, zscore_cap=3.0):
    """
    ...
    for i, date in enumerate(rebalance_dates):
        ...
        # Compute scores
        scores = compute_compass_scores(fund_subset, prices, date, volatility_dict, apply_filters=apply_filters, zscore_cap=zscore_cap)
        if len(scores) == 0:
            continue
...
def main():
    ...
    for cap in caps_to_test:
        print("\n" + "=" * 80)
        print(f"TESTING Z-SCORE CAP: ±{cap}")
        print("=" * 80)

        # Only test OOS to save time (IS not needed for cap comparison)
        oos_result = backtest_period(fund, prices, fwd_returns, volatilities,
                                      OOS_START, OOS_END, f"OOS (cap={cap})",
                                      apply_filters=False, zscore_cap=cap)
        results[cap] = oos_result
    ...
    print("\n" + "=" * 80)
    print("Z-SCORE CAP COMPARISON (OOS 2020-2026)")
    print("=" * 80)

    print(f"\n{'Cap Level':<15} {'Q5-Q1 Spread':<15} {'Stocks':<15}")
    print("-" * 45)

    for cap in caps_to_test:
        r = results[cap]
        if r:
            print(f"±{cap:<13} {r['spread']:>+13.2%} {r['stocks']:>14,}")

    # Find best
    best_cap = max(caps_to_test, key=lambda c: results[c]['spread'] if results[c] else -999)
    print(f"\nBest performing cap: ±{best_cap} with {results[best_cap]['spread']:+.2%} spread")
