#!/usr/bin/env python3
"""
Backtest Compass Score with Quality Filters
===========================================
Compares Compass Score performance with and without quality filters.

Tests:
- Original Compass Score (baseline from research paper)
- Compass Score + Quality Filters (proposed improvement)

Time Periods:
- In-sample: 1995-2019
- Out-of-sample: 2020-2026

Goal: Prove quality filters improve performance without excluding good companies
"""
...
def load_data():
    ...
    print("=" * 80)
    print("COMPASS SCORE QUALITY FILTER BACKTEST")
...
def compute_volatility(prices_df, symbol, date):
    """Compute 60-day price volatility."""
    ...
    recent_prices = sym_prices.tail(60)
...
def compute_compass_scores(fund_subset, prices, date, apply_filters=False):
    """
...
def backtest_period(fund, prices, start_date, end_date, period_name, apply_filters=False):
    """
    ...
    for i, date in enumerate(rebalance_dates):
        ...
        # Compute scores
        scores = compute_compass_scores(fund_subset, prices, date, apply_filters=apply_filters)
        if len(scores) == 0:
            continue
...
def main():
    ...
    is_baseline = backtest_period(fund, prices, IS_START, IS_END, "IN-SAMPLE", apply_filters=False)
    oos_baseline = backtest_period(fund, prices, OOS_START, OOS_END, "OUT-OF-SAMPLE", apply_filters=False)
    ...
    is_filtered = backtest_period(fund, prices, IS_START, IS_END, "IN-SAMPLE", apply_filters=True)
    oos_filtered = backtest_period(fund, prices, OOS_START, OOS_END, "OUT-OF-SAMPLE", apply_filters=True)
